...
def gcp_csv_to_df():
    ...
    blob = bucket.blob("cp_client.csv")
...
# Defining the validation function for data entries of client table as 'val_1_client'
def val_1_client():
    input_csv = gcp_csv_to_df()
    storage_client = storage.Client()

    valid_bucket = storage_client.bucket("abdu_processing")
    blob_valid = valid_bucket.blob("val_1_client.csv")
    invalid_bucket = storage_client.bucket("abdu_error")
    blob_invalid = invalid_bucket.blob("inv_1_client.csv")
