...
def gcp_csv_to_df():
    ...
    blob = bucket.blob("cp_proj.csv")
    data = blob.download_as_string()
    data = data.decode("utf-8")
    data = StringIO(data)
    return data

# Defining the validation function for data entries of project table as 'val_1_proj'
def val_1_proj():
    input_csv = gcp_csv_to_df()
    storage_client = storage.Client()
    valid_bucket = storage_client.bucket("abdu_processing")
    blob_valid = valid_bucket.blob("val_1_proj.csv")
    invalid_bucket = storage_client.bucket("abdu_error")
    blob_invalid = invalid_bucket.blob("inv_1_proj.csv")
